...
public class Consola extends FormateadorFecha {
    ...
    private FactoriaClientes factClientes;
    ...
    public void altaCliente(String datos) {
        ...
        */
        ...
        if (usrInput.equals("P"))
            cliente =factClientes.getParticular(
                    factTarifas.getTarifaBasica(.100d),
                    nombre,
                    genPart.getDNI(),
                    poblacion,
            ...
            cliente = factClientes.getEmpresa(
                    factTarifas.getTarifaBasica(.20d),
    ...
    public void emitirFactura(String datos) {
        ...
        if (!datos.equals("")) {
            ...
            int mes =  Integer.parseInt(datos);
            fact = gestor.emitirFactura(nif, LocalDateTime.now().withMonth(mes));
        } else {
            System.out.print(
            ...
            if (usrInput.equals(""))
                ...
                if (usrInput.matches("(3[10]|[12]?\\\\d+)/(1[0-2]|[1-9])/20(1[0-8]|0\\d)")) {
                    LocalDateTime fecha = LocalDateTime.parse(usrInput.replace("/", "-"));
                    fact = gestor.emitirFactura(nif, LocalDateTime.now());
                } else {
                    ...
                    usrInput.replace("  +", " ");
                    String tokens[] = usrInput.split(" ");
                    fecha1 = LocalDateTime.parse(tokens[0].replace("/", "-"));
                    fecha2 =  LocalDateTime.parse(tokens[1].replace("/", "-"));
    ...
    private LocalDateTime pideFecha(String complemento) {
        ...
        DateFormat format = new SimpleDateFormat("dd/MM/yyyy");
        fecha = LocalDateTime.parse(usrInput.replace("/", "-"));
        return fecha;
    }
